...
const rootDir = './koreksian-wh';

// Definisikan struktur folder dan file yang akan dibuat
const directories = [
  rootDir,
...
const files = {
  ...
  // 2. vite.config.js
  [path.join(rootDir, 'vite.config.js')]: `import { defineConfig } from 'vite'
...
export default defineConfig({
  ...
  [path.join(rootDir, 'tailwind.config.js')]: `/** @type {import('tailwindcss').Config} */
export default {
  content: [
    "./index.html",
    "./src/**/*.{js,ts,jsx,tsx}",
  ...
  // 4. postcss.config.js
  [path.join(rootDir, 'postcss.config.js')]: `export default {
  plugins: {
    tailwindcss: {},
    autoprefixer: {},
  },
  ...
  // 5. index.html
  [path.join(rootDir, 'index.html')]: `<!doctype html>
<html lang="id">
  <head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>Koreksian WH - AI OMR System</title>
  </head>
  ...
  // 6. src/main.jsx
  [path.join(rootDir, 'src', 'main.jsx')]: `import React from 'react'
...
ReactDOM.createRoot(document.getElementById('root')).render(
  ...
  // 7. src/index.css
  [path.join(rootDir, 'src', 'index.css')]: `@tailwind base;
@tailwind components;
@tailwind utilities;
...
@keyframes fadeIn {
  ...
  // 8. src/App.jsx (Full working React code with fixed JSX escaping)
  [path.join(rootDir, 'src', 'App.jsx')]: `import React, { useState, useEffect } from 'react';
import { 
...
// Proses pembuatan direktori
directories.forEach(dir => {
  if (!fs.existsSync(dir)) {
    fs.mkdirSync(dir, { recursive: true });
    console.log(\`[DAERAH] Folder dibuat: \${dir}\`);
  }
});
...
Object.entries(files).forEach(([filePath, content]) => {
  ...
  console.log(\`[SINKRON] File dibuat: \${filePath}\`);
});

console.log('\\n======================================================');
console.log('🎉 PROYEK "KOREKSIAN WH" BERHASIL DI-GENERATE! 🎉');
console.log('======================================================');
console.log('Silakan jalankan langkah berikut di terminal Anda:\\n');
console.log(\`  1. cd \${rootDir}\`);
console.log('  2. npm install');
console.log('  3. npm run dev');
console.log('\\nBuka http://localhost:5173 di browser Anda untuk menguji.');
console.log('======================================================\\n');